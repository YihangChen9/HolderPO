...
def main(
    ...
    dataset_name: str = "./understand_r1_zero_main/datasets/evaluation_suite",
    temperature: float = 0,
    top_p: float = 1,
    max_tokens: int = 3000,
    ...
    max_test: int = 999999,
    save: bool = False,
    metric: str = "avg",  # "avg" -> avg@n (mean of rewards); "pass" -> pass@n (any correct)
    pass_n_tasks: list = None,  # tasks to override with (pass_n_samples, pass_temperature)
    pass_n_samples: int = 8,
    pass_temperature: float = 0.7,
):
    pass_n_tasks = pass_n_tasks or []

    base_seed = int(time.time_ns())

    def make_sampling_params(n, temp):
    ...
    sampling_params = make_sampling_params(n_samples, temperature)
    pass_sampling_params = make_sampling_params(pass_n_samples, pass_temperature)
    ...
    if template in ["qwen_math", "no"]:
        ...
        for sp in (sampling_params, pass_sampling_params):
            sp.stop = ["</answer>"]
    ...
    for task_name, dataset in load_from_disk(dataset_name).items():
        ...
        task_sampling_params = pass_sampling_params if task_name in pass_n_tasks else sampling_params
        print(
            f"inference for {task_name} (n={task_sampling_params.n}, "
            f"temp={task_sampling_params.temperature})"
        )
        ...
        for k in range(len(outputs)):
            output = outputs[k]
            gt_repeated = [targets[k]] * task_sampling_params.n
            rewards, infos = [], []
            ...
            batch_scores_avg.append(rewards.mean())
            batch_scores_pass.append(float((rewards > 0).any()))
        ...
        results_avg[task_name] = np.mean(batch_scores_avg)
        results_pass[task_name] = np.mean(batch_scores_pass)
        ...
        task_n[task_name] = task_sampling_params.n
        print(f"avg@n:", results_avg)
        print(f"pass@n:", results_pass)

    # `metric` selects which one is treated as the primary aggregate (back-compat)
    results = results_pass if metric == "pass" else results_avg
    print("per-task n:", task_n)
    print("avg@n:", results_avg, "| mean:", np.mean(list(results_avg.values())))
    print("pass@n:", results_pass, "| mean:", np.mean(list(results_pass.values())))
    print(f"primary metric ({metric}):", results)
